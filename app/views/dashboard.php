...
<style>
      ::-webkit-scrollbar{width:10px;height:10px;background-color:#ccc}::-webkit-scrollbar-track{background:#ccc}::-webkit-scrollbar-track-piece{background:#ccc;height:20px}::-webkit-scrollbar-thumb{background:#333;border-radius:3px}::-webkit-scrollbar-corner{background:red;border-radius:5px}
      </style>
    </head> 
      
    <body onload="exibeChart()" class="loading" data-layout-color="<?= custom($AuthUser->get("id"), "modo_tema") == "" ? "light" : custom($AuthUser->get("id"), "modo_tema") ?>" data-leftbar-theme="<?= custom($AuthUser->get("id"), "cor_menu") == "" ? "default" : custom($AuthUser->get("id"), "cor_menu") ?>" data-layout-mode="<?= custom($AuthUser->get("id"), "largura_tema") == "" ? "fixed" : custom($AuthUser->get("id"), "largura_tema") ?>" data-rightbar-onstart="true">
