...
<!-- END: Head-->
    <body   class="loading" data-layout-color="<?= custom($AuthUser->get("id"), "modo_tema") == "" ? "light" : custom($AuthUser->get("id"), "modo_tema") ?>" data-leftbar-theme="<?= custom($AuthUser->get("id"), "cor_menu") == "" ? "default" : custom($AuthUser->get("id"), "cor_menu") ?>" data-layout-mode="<?= custom($AuthUser->get("id"), "largura_tema") == "" ? "fixed" : custom($AuthUser->get("id"), "largura_tema") ?>" data-rightbar-onstart="true">
         ...
         <div class="content-page">
          <div class="content">
            <div class="container-fluid">
                    ...
                    <?php require(APPPATH.'/views/fragments/analise/modal-analise.fragment.php'); ?>

                    <?php require(APPPATH.'/views/fragments/relatorio-tiino.fragment.php'); ?>
                   </div>
         </div>
        </div>
        ...
        <script src="<?= APPURL . "/assets/js/vendor/jquery-jvectormap-world-mill-en.js"?>"></script>
			  ...
			  <script src="<?= APPURL . "/assets/js/custom/usuario.js"?>"></script>
    ...
    $("#solicita").on("click", function(e){ 
  e.preventDefault();


  var cnpj = $("#cnpj").val();
  var valor = $("#valor").val();
  var valor = valor.replace(/[^0-9]/g,'');
  ...
  var moeda = (valor / 100);
      ...
      $.ajax({
             ...
             if(resp.result == 1){
              ...
              position: 'top-end',
              icon: 'success',
              title: 'Aguarde no seu e-mail a verificação para o CNPJ informado!',
            ...
            $("#valor").val("");
              ...
              position: 'center',
              icon: 'info',
              title: 'Verifique se o dado informando, é do tipo CPNJ!',
              showConfirmButton: true,
             // timer: 5000
            })          
            }   
