<?php 
	...
	class MotivoModel extends DataEntry
	{	
		/**
	    ...
	    public function select($uniqid)
	    {
	    	...
	    	if ($col) {
		    	$query = DB::table(TABLE_PREFIX.TABLE_CONFIG_MOTIVOS)
			    	      ->where($col, "=", $uniqid)
			    	      ->limit(1)
			    	      ->select("*");
		    	if ($query->count() == 1) {
	    ...
	    public function insert()
	    {
	    	...
	    	$id = DB::table(TABLE_PREFIX.TABLE_CONFIG_MOTIVOS)
		    	->insert(array(
		    		"id" => null,
		    		"status" => $this->get("status"),
		    		"motivo" => $this->get("motivo"),
						"funil" => $this->get("funil"),
	    ...
	    public function delete()
	    {
	    	...
	    	DB::table(TABLE_PREFIX.TABLE_CONFIG_MOTIVOS)->where("id", "=", $this->get("id"))->delete();
	    	$this->is_available = false;
	    	return true;
	    }   
