<?php 
	...
	class CentroCustoModel extends DataEntry
	{	
		/**
	    ...
	    public function select($uniqid)
	    {
	    	...
	    	if ($col) {
		    	$query = DB::table(TABLE_PREFIX.TABLE_CENTRO_CUSTOS)
			    	      ->where($col, "=", $uniqid)
			    	      ->limit(1)
			    	      ->select("*");
		    	if ($query->count() == 1) {
	    ...
	    public function insert()
	    {
	    	...
	    	$id = DB::table(TABLE_PREFIX.TABLE_CENTRO_CUSTOS)
		    	->insert(array(
		    		"id" => null,
	    ...
	    public function delete()
	    {
	    	...
	    	DB::table(TABLE_PREFIX.TABLE_CENTRO_CUSTOS)->where("id", "=", $this->get("id"))->delete();
	    	$this->is_available = false;
	    	return true;
	    }   
