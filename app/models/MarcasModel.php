 ...
 * 
 */
class MarcasModel extends DataList
{	
	/**
	 * Initialize
	 */
	...
	{
		$this->setQuery(DB::table(TABLE_PREFIX.TABLE_MARCAS));        
	}
    ...
    public function search($search_query)
    {
        ...
        foreach ($search_strings as $sq) {
            ...
            $query->where(function($q) use($sq) {               
                $q->orWhere(TABLE_PREFIX.TABLE_MARCAS.".nome", "LIKE", $sq."%");                
            });
        }
    ...
    public function fetchData()
    {
        ...
        $this->getQuery()
             ->select(TABLE_PREFIX.TABLE_MARCAS.".*");             
